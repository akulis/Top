angular.module('OrderCloud-ProductLightbox', ['ngTouch', 'ui.bootstrap']);

angular
  .module('OrderCloud-ProductLightbox')
...
function productlightbox() {
  ...
  function template() {
    return [
      ...
      '.scrollLightbox {width: 450px;height: 75px;overflow: auto;white-space: nowrap;}',
      '.product-image-large {margin: 0 auto;max-width: 390px;}',
      '</style>',
    ...
    if($scope.LineItem.Specs || $scope.LineItem.Product && $scope.LineItem.Product.StaticSpecGroups) {
      ...
      if($scope.LineItem.Product.StaticSpecGroups[specGroupName]) {
        ...
        angular.forEach($scope.LineItem.Product.StaticSpecGroups[specGroupName].Specs, function(staticSpecs) {
            ...
            if((staticSpec && specOption) && (staticSpec.toLowerCase() === specOption.toLowerCase()) {
                image.Selected = true;
              }
              $scope.LineItem.images.push(image); count++;
            });
        }
